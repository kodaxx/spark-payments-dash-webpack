...
export default {
    ...
    price: {
      ...
      set: function (newValue) {
        if (newValue.length > 2) {
          ...
          this.native = `${newValue.substr(0, newValue.length - 2)}.${newValue.substr(newValue.length - 2)}`
        } else {
  ...
  methods: {
    setTwoNumberDecimal: function (e) {
      this.native = parseFloat(this.native).toFixed(2)
    },
    moveCursor: function () {
    ...
    donate: function () {
      ...
      if (this.native === '' || parseFloat(this.native === 0)) { // string vs int?
        swal('Error!', this.language.errors.price, 'error')
        return
      }
      // show QR page and pass data
      router.push(`/sale/${this.price}%20${this.currency}?address=${this.$route.query.address}`)
    }
  },
